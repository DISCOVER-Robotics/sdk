...
PYBIND11_MODULE(airbot, m) {
  ...
  py::class_<Robot, std::unique_ptr<Robot>>(m, "Robot", DOC(Robot))
      .def("alter_logging", &Robot::alter_logging, DOC(Robot, alter_logging))
      .def("get_target_pose", &Robot::get_target_pose, DOC(Robot, get_target_pose))
      .def("get_target_joint_q", &Robot::get_target_joint_q, DOC(Robot, get_target_joint_q))
      .def("get_target_joint_v", &Robot::get_target_joint_v, DOC(Robot, get_target_joint_v))
      .def("get_target_joint_t", &Robot::get_target_joint_t, DOC(Robot, get_target_joint_t))
      .def("get_target_translation", &Robot::get_target_translation, DOC(Robot, get_target_translation))
      .def("get_target_rotation", &Robot::get_target_rotation, DOC(Robot, get_target_rotation))
      .def("get_current_pose", &Robot::get_current_pose, DOC(Robot, get_current_pose))
      .def("get_current_joint_q", &Robot::get_current_joint_q, DOC(Robot, get_current_joint_q))
      .def("get_current_joint_v", &Robot::get_current_joint_v, DOC(Robot, get_current_joint_v))
      .def("get_current_joint_t", &Robot::get_current_joint_t, DOC(Robot, get_current_joint_t))
      .def("get_current_translation", &Robot::get_current_translation, DOC(Robot, get_current_translation))
      .def("get_current_rotation", &Robot::get_current_rotation, DOC(Robot, get_current_rotation))
      .def("get_current_end", &Robot::get_current_end, DOC(Robot, get_current_end))
      .def("get_current_joint_error_code", &Robot::get_current_joint_error_code,
           DOC(Robot, get_current_joint_error_code))
      .def("get_current_joint_temperature", &Robot::get_current_joint_temperature,
           DOC(Robot, get_current_joint_temperature))
      .def("get_motor_response_cnt", &Robot::get_motor_response_cnt, DOC(Robot, get_motor_response_cnt))
      .def("get_sn", &Robot::get_sn, DOC(Robot, get_sn))
      .def("set_target_pose",
      ...
      .def("add_target_translation", &Robot::add_target_translation, py::arg("target_d_translation"),
           py::arg("use_planning") = true, py::arg("time") = 1., DOC(Robot, add_target_translation))
      .def("add_target_relative_translation", &Robot::add_target_relative_translation, py::arg("target_d_translation"),
           py::arg("use_planning") = true, py::arg("time") = 1., DOC(Robot, add_target_relative_translation))
      .def("set_target_rotation", &Robot::set_target_rotation, py::arg("target_rotation"),
           py::arg("use_planning") = true, py::arg("time") = 1., DOC(Robot, set_target_rotation))
      .def("add_target_relative_rotation", &Robot::add_target_relative_rotation, py::arg("target_d_rotation"),
           py::arg("use_planning") = true, py::arg("time") = 1., DOC(Robot, add_target_relative_rotation))
      .def("set_target_vel", &Robot::set_target_vel, py::arg("target_vel"), DOC(Robot, set_target_vel))
      .def("set_target_joint_q", &Robot::set_target_joint_q, py::arg("target_joint_q"), py::arg("use_planning") = true,
           py::arg("time") = 1., DOC(Robot, set_target_joint_q))
      .def("add_target_joint_q", &Robot::add_target_joint_q, py::arg("target_d_joint_q"),
           py::arg("use_planning") = true, py::arg("time") = 1., DOC(Robot, add_target_joint_q))
      .def("set_target_joint_v", &Robot::set_target_joint_v, py::arg("target_joint_v"), DOC(Robot, set_target_joint_v))
      .def("add_target_joint_v", &Robot::add_target_joint_v, py::arg("target_d_joint_v"),
           DOC(Robot, add_target_joint_v))
      .def("set_target_joint_t", &Robot::set_target_joint_t, py::arg("target_joint_t"), DOC(Robot, set_target_joint_t))
      .def("set_target_end", &Robot::set_target_end, py::arg("target_end"), DOC(Robot, set_target_end))
      .def("record_start", &Robot::record_start, DOC(Robot, record_start))
      .def("record_stop", &Robot::record_stop, DOC(Robot, record_stop))
      .def("record_replay", &Robot::record_replay, DOC(Robot, record_replay))
      .def("record_save", &Robot::record_save, py::arg("filepath"), DOC(Robot, record_save))
      .def("record_load", &Robot::record_load, py::arg("filepath"), DOC(Robot, record_load))
      .def("gravity_compensation", &Robot::gravity_compensation, DOC(Robot, gravity_compensation))
      .def("stop_gravity_compensation", &Robot::stop_gravity_compensation, DOC(Robot, stop_gravity_compensation))
