...
PYBIND11_MODULE(airbot, m) {
  ...
  py::class_<arm::Robot<6>, std::unique_ptr<arm::Robot<6>>>(m, "arm::Robot<6>", DOC(arm::Robot<6>))
      .def("logging", &arm::Robot<6>::logging, py::arg("logging"), DOC(arm::Robot<6>, logging))
      .def("get_current_pose", &arm::Robot<6>::get_current_pose, DOC(arm::Robot<6>, get_current_pose))
      .def("get_current_joint_q", &arm::Robot<6>::get_current_joint_q, DOC(arm::Robot<6>, get_current_joint_q))
      .def("get_current_joint_v", &arm::Robot<6>::get_current_joint_v, DOC(arm::Robot<6>, get_current_joint_v))
      .def("get_current_joint_t", &arm::Robot<6>::get_current_joint_t, DOC(arm::Robot<6>, get_current_joint_t))
      .def("get_current_translation", &arm::Robot<6>::get_current_translation,
           DOC(arm::Robot<6>, get_current_translation))
      .def("get_current_rotation", &arm::Robot<6>::get_current_rotation, DOC(arm::Robot<6>, get_current_rotation))
      .def("get_current_end", &arm::Robot<6>::get_current_end, DOC(arm::Robot<6>, get_current_end))
      .def("get_current_joint_error_code", &arm::Robot<6>::get_current_joint_error_code,
           DOC(arm::Robot<6>, get_current_joint_error_code))
      .def("get_current_joint_temperature", &arm::Robot<6>::get_current_joint_temperature,
           DOC(arm::Robot<6>, get_current_joint_temperature))
      //  .def("get_motor_response_cnt", &arm::Robot<6>::get_motor_response_cnt, DOC(arm::Robot<6>,
      //  get_motor_response_cnt))
      .def("get_sn", &arm::Robot<6>::get_sn, DOC(arm::Robot<6>, get_sn))
      ...
      .def("set_target_pose", py::overload_cast<const Frame &, bool, double, bool>(&arm::Robot<6>::set_target_pose),
           ...
           DOC(arm::Robot<6>, set_target_pose))
      .def(
          "set_target_pose",
          ...
          py::arg("vel") = 0.2, py::arg("blocking") = false, DOC(arm::Robot<6>, set_target_pose))
      ...
      .def("set_target_joint_mit",
           ...
           DOC(arm::Robot<6>, set_target_joint_mit))
      .def("set_target_joint_mit",
           py::overload_cast<const Joints<6> &, const Joints<6> &, const Joints<6> &, const Joints<6> &,
                             const Joints<6> &>(&arm::Robot<6>::set_target_joint_mit),
           py::arg("target_joint_q"), py::arg("target_joint_v"), py::arg("target_joint_kp"), py::arg("target_joint_kd"),
           py::arg("target_joint_t"), DOC(arm::Robot<6>, set_target_joint_mit))
      .def("set_target_end", &arm::Robot<6>::set_target_end, py::arg("target_end"), py::arg("blocking") = false,
           DOC(arm::Robot<6>, set_target_end))

      .def("valid_target_pose", &arm::Robot<6>::valid_target_pose, py::arg("target_pose"),
           DOC(arm::Robot<6>, valid_target_pose))
      .def("valid_joint_q", &arm::Robot<6>::valid_joint_q, py::arg("joint_q"), DOC(arm::Robot<6>, valid_joint_q))
      .def("safe_joint_q", &arm::Robot<6>::safe_joint_q, py::arg("joint_q"), DOC(arm::Robot<6>, safe_joint_q))

      .def("record_save", &arm::Robot<6>::record_save, py::arg("filepath"), DOC(arm::Robot<6>, record_save))
      .def("record_load", &arm::Robot<6>::record_load, py::arg("filepath"), DOC(arm::Robot<6>, record_load))
      .def("record_start", &arm::Robot<6>::record_start, DOC(arm::Robot<6>, record_start))
      .def("record_stop", &arm::Robot<6>::record_stop, DOC(arm::Robot<6>, record_stop))

      .def("replay_start", &arm::Robot<6>::replay_start, DOC(arm::Robot<6>, replay_start))

      .def("manual_mode", &arm::Robot<6>::manual_mode, DOC(arm::Robot<6>, manual_mode))
      .def("offline_mode", &arm::Robot<6>::offline_mode, DOC(arm::Robot<6>, offline_mode))
      .def("online_mode", &arm::Robot<6>::online_mode, DOC(arm::Robot<6>, online_mode))
      .def("reset_error", &arm::Robot<6>::reset_error, DOC(arm::Robot<6>, reset_error))
