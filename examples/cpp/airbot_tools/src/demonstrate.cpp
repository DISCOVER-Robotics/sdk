  ...
  if (!fs::exists(path))
  {
    try
    {
      if (fs::create_directories(path))
        print("Directory created: " + std::string(path), 2, 0);
    }
...
int main(int argc, char **argv)
{
  ...
  for (auto &&ct : camera_threads_init)
    ct.join();
      ...
      else
      {
        ...
        for (std::size_t i = 0; i < camera_num; i++)
          camera_saving_threads.emplace_back(std::thread(
              ...
              i));
        for (auto &&ctd : camera_saving_threads)
          ctd.join();
        series_count++;
        clear_data();
  ...
  for (auto &&i : threads)
    i.join();
  auto reset_leader = std::thread([&]()
                                  { leader.reset(); });
  auto reset_follower = std::thread([&]()
                                    { follower.reset(); });
  reset_leader.join();
  reset_follower.join();
  for (auto &&i : camera_threads)
    i.join();
  endwin();
  return 0;
}
