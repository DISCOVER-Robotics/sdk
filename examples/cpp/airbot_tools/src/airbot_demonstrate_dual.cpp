    ...
    switch (ch) {
      case ' ':  // 空格按下开始记录，再按下结束记录保存数据
        if (!recording) {
          ...
          leader_left->gravity_compensation();
          leader_right->gravity_compensation();
          print("", 1, 0);
          print("", 2, 0);
        } else {
          if (qt_records_left_.size() < max_time_steps || qt_records_right_.size() < max_time_steps) break;
          recording = false;
          leader_left->stop_gravity_compensation();
          leader_right->stop_gravity_compensation();
      ...
      case 'p':  // 打印当前位置
        ...
        print("Current eef pose of left: " + std::to_string(leader_left->get_current_pose()[0][0]) + " " +
                  ...
                  std::to_string(leader_left->get_current_pose()[0][2]) + " " +
                  std::to_string(leader_left->get_current_pose()[1][0]) + " " +
                  std::to_string(leader_left->get_current_pose()[1][1]) + " " +
                  std::to_string(leader_left->get_current_pose()[1][2]) + " " +
                  std::to_string(leader_left->get_current_pose()[1][3]),
              4, 0);
        print("Current eef pose of right: " + std::to_string(leader_right->get_current_pose()[0][0]) + " " +
                  std::to_string(leader_right->get_current_pose()[0][1]) + " " +
                  std::to_string(leader_right->get_current_pose()[0][2]) + " " +
                  std::to_string(leader_right->get_current_pose()[1][0]) + " " +
                  std::to_string(leader_right->get_current_pose()[1][1]) + " " +
                  std::to_string(leader_right->get_current_pose()[1][2]) + " " +
                  std::to_string(leader_right->get_current_pose()[1][3]),
              5, 0);
        break;
      default:
        break;
