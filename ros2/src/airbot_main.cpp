...
class AirbotROS2Interface : public rclcpp::Node {
 public:
  AirbotROS2Interface() : Node("airbot_arm_ros") {
    ...
    air_robot_ = std::make_unique<arm::Robot>(
        std::make_unique<arm::AnalyticFKSolver>(urdf_path_), std::make_unique<arm::AnalyticIKSolver>(urdf_path_),
        std::make_unique<arm::ChainIDSolver>(urdf_path_, "down"), can_if_.c_str(), M_PI / 2, end_mode_, false, false);
    ...
    arm_pose_cmd_sub_ = this->create_subscription<Pose>("/airbot_play/pose_cmd", QoS_CONSTANT, [this](PosePtr pose) {
      this->air_robot_->set_target_pose(
    ...
    endeffector_cmd_sub_ = this->create_subscription<Bool>(
        "/airbot_play/gripper/state_cmd", QoS_CONSTANT,
        [this](BoolPtr end) { this->air_robot_->set_target_end(std::clamp(end->data ? 0.99 : 0.01, 0.0, 1.0)); });

    bool recording = false;
    joy_cmd_sub_ = this->create_subscription<Joy>("/joy_latched", QoS_CONSTANT, [this](const JoyPtr &joy) {
      bool flag = false;
      ...
      if (joy->buttons[7] == 0) {
        if (std::abs(joy->axes[1]) > 1e-3 || std::abs(joy->axes[0]) > 1e-3 || std::abs(joy->axes[3]) > 1e-3) {
          if (joy->buttons[6] == 1) {  // LT
            ...
            this->air_robot_->add_target_translation(
                {scale * joy->axes[1], scale * joy->axes[0], scale * joy->axes[3]});
          }
        }
      } else {
    ...
    joy_diff_cmd_sub_ =
        this->create_subscription<Joy>("/joy_trigger", QoS_CONSTANT, [this, &recording](const JoyPtr &joy) {
          if (joy->buttons[3] == 1) this->air_robot_->gravity_compensation();  // Y
          if (joy->buttons[0] == 1) {
          ...
          if (joy->buttons[2] == 1) this->air_robot_->record_replay();  // B
        });
